#!/usr/bin/env bash
# Provision an Azure Ubuntu VM with Terraform, then install Podman with Ansible.
# Prototype parallel to vm/az/01_az_vm.sh (az CLI). Must be sourced.
...
_ANSIBLE_DIR="${_VM_AZ_DIR}/ansible"
_SSH_DIR="${_TF_DIR}/.ssh"
_INVENTORY_FILE="${_ANSIBLE_DIR}/inventory.ini"

# #############################################################################
# Prereqs

for _bin in terraform ansible-playbook az ssh; do
...
echo -e "\nTerraform init + apply (Azure VM)\n"
echo -e   "---------------------------------\n"

(
  cd "${_TF_DIR}" || exit 1
  terraform init -upgrade || exit 1
  terraform apply -auto-approve -var-file=terraform.tfvars || exit 1
) || return 1
...
# #############################################################################
# Ansible inventory + Podman playbook

mkdir -p "${_ANSIBLE_DIR}"
cat > "${_INVENTORY_FILE}" <<EOF
[vm]
${VM_HOST_FQDN} ansible_user=${VM_ADMIN_USERNAME} ansible_ssh_private_key_file=${VM_SSH_PRIVATE_KEY} ansible_ssh_common_args='-o StrictHostKeyChecking=accept-new'
EOF

echo -e "\nAnsible: install Podman\n"
echo -e   "-----------------------\n"

ansible-playbook -i "${_INVENTORY_FILE}" "${_ANSIBLE_DIR}/playbook-podman.yml" || return 1
