 ...
 *
 * USAGE: Run in the TARGET database (your application/user database), not in master.
 *        On Azure SQL and GCP Cloud SQL, CREATE OBJECT is not allowed in master.
 *        Example: DB_CATALOG='your_database_name' ... | SQLCLI_DBA (or pipe this script).
 *
 * CHANGE HISTORY:
 ...
 *   - Capture instances are now created with lakeflow naming (lakeflow_schema_table_1)
 *     for better identification and management
...
BEGIN
    ...
    -- db_owner in this database, sysadmin at server, or cloud DBA login (e.g. GCP/Azure 'sqlserver' in master)
    IF (IS_ROLEMEMBER('db_owner') = 0 AND IS_SRVROLEMEMBER('sysadmin') = 0
        AND NOT (DB_NAME() = 'master' AND SUSER_SNAME() = 'sqlserver'))
        BEGIN
...
BEGIN
    ...
    ELSE IF DB_ID(''msdb'') IS NOT NULL AND OBJECT_ID(''msdb.dbo.gcloudsql_cdc_enable_db'', ''P'') IS NOT NULL
        SET @platform = ''GCP_CLOUD_SQL'';
...
PRINT N'Creating lakeflowUtilityVersion_1_4 function...';
EXEC sp_executesql N'
CREATE FUNCTION dbo.lakeflowUtilityVersion_1_4()
RETURNS NVARCHAR(10)
AS
BEGIN
    RETURN ''1.4'';
END';
PRINT N'Created lakeflowUtilityVersion_1_4 function';
...
BEGIN
    ...
    BEGIN TRY
        -- Validate that current user is db_owner, sysadmin, or cloud DBA (sqlserver in master)
        IF (IS_ROLEMEMBER(''db_owner'') = 0 AND IS_SRVROLEMEMBER(''sysadmin'') = 0
            AND NOT (DB_NAME() = ''master'' AND SUSER_SNAME() = ''sqlserver''))
        BEGIN
...
BEGIN
    ...
    DECLARE @ErrorMessage NVARCHAR(4000);
    DECLARE @SQL NVARCHAR(MAX);
    DECLARE @versionSuffix NVARCHAR(10) = ''_1_4'';
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            -- Use platform-specific CDC enablement procedure (dynamic EXEC to avoid dependency on missing object on other platforms)
            DECLARE @currentDbName NVARCHAR(128) = DB_NAME();
            DECLARE @cdcExecCmd NVARCHAR(500);
            IF @Platform = ''GCP_CLOUD_SQL''
            BEGIN
                IF OBJECT_ID(''msdb.dbo.gcloudsql_cdc_enable_db'', ''P'') IS NULL
                BEGIN
                    RAISERROR(''Platform detected as GCP Cloud SQL but required procedure msdb.dbo.gcloudsql_cdc_enable_db does not exist'', 16, 1);
                    ROLLBACK TRAN;
                    RETURN;
                END
                SET @cdcExecCmd = N''EXEC msdb.dbo.gcloudsql_cdc_enable_db @db'';
                EXEC sp_executesql @cdcExecCmd, N''@db NVARCHAR(128)'', @db = @currentDbName;
            END
            ELSE IF @Platform = ''AMAZON_RDS''
            BEGIN
                IF OBJECT_ID(''msdb.dbo.rds_cdc_enable_db'', ''P'') IS NULL
                BEGIN
                    RAISERROR(''Platform detected as Amazon RDS but required procedure msdb.dbo.rds_cdc_enable_db does not exist'', 16, 1);
                    ROLLBACK TRAN;
                    RETURN;
                END
                SET @cdcExecCmd = N''EXEC msdb.dbo.rds_cdc_enable_db @db_name = @db'';
                EXEC sp_executesql @cdcExecCmd, N''@db NVARCHAR(128)'', @db = @currentDbName;
            END
            ELSE
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        BEGIN
                            ...
                            -- Trigger reinit instead of dropping instances here to avoid race conditions
                            -- Cleanup will be handled by Replicant code after reinit completes
                            SET @TriggerReinit = 1;
                            SET @SkipCaptureInstanceCreation = 1;

                            PRINT ''''Both capture instance slots occupied by lakeflow instances. Triggering reinit for safe schema migration on table '''' + @QuotedFullName;
                        END
                    END
...
BEGIN
    ...
    DECLARE @Platform NVARCHAR(50) = dbo.lakeflowDetectPlatform();
    PRINT N'Platform: ' + @Platform;
    PRINT N'Version: ' + dbo.lakeflowUtilityVersion_1_4();
    ...
    IF OBJECT_ID('dbo.lakeflowUtilityVersion_1_4', 'FN') IS NOT NULL
        PRINT N'✓ lakeflowUtilityVersion_1_4 function created successfully'
    ELSE
        PRINT N'✗ lakeflowUtilityVersion_1_4 function creation failed';
