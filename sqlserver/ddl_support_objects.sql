 ...
 *
 * SQL Server DDL Support Objects Script for Databricks Lakeflow Connect
 * Version 1.3
 *
 * CHANGELOG:
 * Version 1.3:
 *   - Added lakeflow capture instance naming convention (lakeflow_schema_table_1/2)
 *   - Enhanced capture instance management for 2-instance SQL Server limit
 *   - Added support for preserving pre-existing (non-lakeflow) capture instances
 *
 * Version 1.2:
 *   - Added @reinit parameter to skip merge during reinit (performance optimization)
 *
 * Version 1.1:
 *   - Initial DDL support objects release
 */
...
BEGIN
    ...
    IF (@mode = 'BOTH' OR @mode = 'CT')
        ...
        SET @ctEnabled = (SELECT ctd.database_id FROM sys.change_tracking_databases ctd
                          INNER JOIN sys.databases d ON ctd.database_id = d.database_id
                          WHERE d.name = @currentCatalogName);
    ...
    BEGIN TRY
        EXECUTE ('
            ...
            SELECT @DropScript = @DropScript + ''DROP PROCEDURE '' + QUOTENAME(schema_name(schema_id))+ ''.'' + QUOTENAME(name) + '';''
            FROM sys.procedures
            ...
            EXECUTE ('
                    ...
                    AS
                        ...
                        BEGIN TRAN
                                ...
                                BEGIN
                                    ...
                                    IF (@LakeflowInstanceCount = 2)
                                        BEGIN
                                            IF (@reinit = 1)
                                            BEGIN
                                                -- During reinit recovery, do not trigger reinit again
                                                SET @TriggerReinit = 0;

                                                -- During reinit, if we have 2 lakeflow instances, drop the oldest one
                                                DECLARE @OldestLakeflowInstanceForReinit NVARCHAR(MAX);
                                            ...
                                            ELSE
                                            BEGIN
                                                -- During DDL refresh (not reinit), both instances are lakeflow instances (TriggerReinit already set above)
                                    ...
                                    ELSE IF (@LakeflowInstanceCount = 1)
                                        BEGIN
                                            -- One lakeflow instance and one pre-existing instance (TriggerReinit already set above)
                                            ...
                                            -- Skip creating a new instance immediately - wait for full refresh/reinit
                                            SET @SkipCaptureInstanceCreation = 1;
                                        END
                                    ELSE
                                        BEGIN
                                            -- Both slots are taken by non-lakeflow instances (TriggerReinit already set above)
                            ...
                            SET @OldCaptureInstanceTableName = ''[cdc].'' + QUOTENAME(@OldCaptureInstance + ''_CT'');
                            SET @NewCaptureInstanceTableName = ''[cdc].'' + QUOTENAME(@NewCaptureInstance + ''_CT'');
                            SET @DDLHistoryTable = ''[cdc].[ddl_history]'';

                            DECLARE @CommittedCursor VARCHAR(MAX);
                            DECLARE @OldInstanceToTrack nvarchar(max);
                            ...
                            IF @SkipCaptureInstanceCreation = 0
                                BEGIN
                                    BEGIN TRAN
                                        ...
                                        IF (@TriggerReinit = 0 AND @reinit = 0 AND @OldInstanceToTrack IS NOT NULL)
                                            BEGIN
